...
async function loadBranding() {
  ...
  if (!b) return;
  brandingState = b;
  $("bvName").value = b.app_name === "InboxAI" ? "" : b.app_name;
...
function previewBranding() {
  ...
  const name = $("bvName").value.trim() || "InboxAI";
...
$("bvLogoFile").addEventListener("change", async (e) => {
  ...
  if (r?.logo) { renderLogoPreview(r.logo); previewBranding(); applyBranding({ ...brandingState, logo: r.logo, hue: $("bvHue").value, app_name: $("bvName").value.trim() || "InboxAI" }); toast("Logo uploaded", "success"); }
  else toast(r?.error || "Upload failed", "error");
  e.target.value = "";
});
$("bvLogoRemove").addEventListener("click", async () => {
  ...
  applyBranding({ ...brandingState, logo: "", hue: $("bvHue").value, app_name: $("bvName").value.trim() || "InboxAI" });
});

$("bvSave").addEventListener("click", async () => {
  const body = {
    app_name: $("bvName").value.trim() || "InboxAI",
...
$("reportBtn").addEventListener("click", async () => {
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>Report — ${escHtml(d.org || b.app_name)}</title>
    ...
    <div class="hd"><div class="logo">${b.logo ? `<img src="${b.logo}" style="height:34px">` : escHtml(b.app_name || "InboxAI")}</div>
    ...
    <div class="ft">Generated by ${escHtml(b.app_name || "InboxAI")} · ${new Date(d.generatedAt).toLocaleString()}</div>
...
/* ============================================================
   ...
   ============================================================ */
const TOUR_STEPS = [
  { title: "Welcome to InboxAI 🎉", text: "Your WhatsApp AI inbox is ready. This quick tour shows you the key features.", art: `<svg width="80" height="80" viewBox="0 0 80 80"><circle cx="40" cy="40" r="36" fill="oklch(0.95 0.03 158)"/><text x="40" y="52" text-anchor="middle" font-size="32">📱</text></svg>` },
